module Rpio
  ...
  class PWM
    ...
    def initialize(options)
      ...
      Rpio.driver.pwm_set_pin(@options[:pin])
      Rpio.driver.pwm_set_clock(get_clock_divider(@options[:clock]))
      Rpio.driver.pwm_set_range(@options[:pin], options[:range])

      self.value= @options.delete(:value)

      Rpio.driver.pwm_set_mode(@options[:pin], options[:mode])
    ...
    def on
      @on = true
      Rpio.driver.pwm_set_mode(@options[:pin], options[:mode], 1) #check if compatible with other driver than bcm2835
    end
    ...
    def value=(new_value)
      ...
      Rpio.driver.pwm_set_data(@options[:pin], data(@value))
    end

    private

    def get_clock_divider(clock)
      ((19.2*10**6).to_f / clock).to_i
    end
